...
{
    public class BiohazardBlaster : EBFGun, ILocalizedModType
    {
        public new string LocalizationCategory => "Items.Weapons.Ranged.Guns";
        public override void SetDefaults()
        {
            base.SetDefaults();
            launcherType = ModContent.ProjectileType<BiohazardBlasterLauncher>();
            sidearmType = ModContent.ProjectileType<BiohazardBlasterSidearm>();
            overheatTime = 60 * 12;
    ...
    public class BiohazardCloud : ModProjectile
    {
        ...
        public override void AI()
        {
            if (Main.GameUpdateCount % 2 == 0)
            {
                ...
                {
                    Projectile.Resize(Projectile.width + 2, Projectile.height + 2);
                }
                ...
                for (int i = 0; i < 1 + Projectile.width / 50; i++)
                {
                    ...
                    Gore gore = Gore.NewGorePerfect(Projectile.GetSource_FromThis(), position, Main.rand.NextVector2Square(-1f, 1f), Type: Main.rand.Next(435, 438), Scale: 0.5f + ((float)Projectile.width * 2 / maxSize));
                    gore.alpha = 128;
