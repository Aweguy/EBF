    ...
    public class ThornBow_Arrow : EBFChargeableArrow
    {
        ...
        public override void OnProjectileRelease()
        {
            if (FullyCharged)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                   Projectile Stinger = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity.RotatedByRandom(0.2d), ProjectileID.HornetStinger, Projectile.damage, Projectile.knockBack, Projectile.owner);
                    Stinger.DamageType = DamageClass.Ranged;
                }
            }
        }
    }
