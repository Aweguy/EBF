    ...
    public class FairyBow_Arrow : EBFChargeableArrow
    {
        ...
        public override void OnProjectileRelease()
        {
            if (FullyCharged)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    Projectile proj = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity.RotatedByRandom(0.33d), ProjectileID.Leaf, Projectile.damage, Projectile.knockBack, Projectile.owner);
                }
            }
        }
    }
